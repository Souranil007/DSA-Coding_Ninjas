...
void check(int input[], int size, int k, int output[], int m){
    if(size==0){
        if(k==0){
            for(int i=0; i<m; i++){
                cout<<output[i]<<"";
    ...
    output1[i] = input[0];
    check(input+1, size-1, k-input[0], output1, m+1);
    check(input+1, size-1, k, output, m);
}
